...
namespace BackTestingPlatform.Transaction.TickTransaction
{
    ...
    public class TickTransactionWtihSlip
    {
        ...
        //       /// <summary>
        //       /// 该成交判断方法，开仓与平仓分开
        //       /// （1）开仓行为一般是给定金额限制，滑点存在时实际成交量会变化；而平仓行为一般是给定需要成交的数量限制，滑点//存在时实际成交金额会变化，两者成交方式有所区别，似乎分开更好；（2）开仓需检查freeCash，平仓检查持仓
        //       /// </summary>
        //
        ...
        //                       //若信号为short，且调整持仓后交易总量大于等于空头持仓量,说明是先平多再开空，只收取平多部分手/续/费
        ...
        //                   //当前品种总现金流，包含历史现金流，若未持仓该品种，则记录持仓盈亏，若有持仓，则为历史现金流 + 当//前现金流。该指标用于计算freeCash
